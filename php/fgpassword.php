...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
    ...
    if(!empty($name) && !empty($contact) && !empty($email) && !empty($new_password) && ($new_password == $confirm_password))
    {
        ...
        $query = "select * from admin where name='$name' and contact='$contact' and email='$email' limit 1";
        $result = mysqli_query($con, $query);

        if($result && mysqli_num_rows($result) > 0)
        {
            // update the password
            $query = "update admin set password='$new_password' where name='$name' and contact='$contact' and email='$email'";
...
<div id="box">
    <form method="post">
        <div style="text-align: center;">
            <img src="../img/img/forgot_password.png" alt="User Icon" width="100" height="100" />
        </div>
